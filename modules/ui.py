...
def get_available_cameras() -> Tuple[List[int], List[str]]:
    ...
    if platform.system() == "Darwin":
        try:
            ...
            cams = []
        if cams:
            return [cam.index for cam in cams], [cam.name for cam in cams]
        return [], ["No cameras found"]
...
class WebcamPreviewWindow(QWidget):
    def __init__(self, camera_index: int):
        ...
        self.sw_virtual_cam = _Switch(
            _("Send to Virtual Camera"), False,
            _("Publish the swapped feed to a system virtual camera (OBS Virtual Camera "
              "on macOS) so other apps like Zoom or Meet can use it as a webcam"),
        )
        self.sw_virtual_cam.toggled.connect(self._on_virtual_cam_toggled)
        if not HAS_PYVIRTUALCAM:
            self.sw_virtual_cam.setEnabled(False)
    ...
    def _tick(self) -> None:
        ...
        try:
            ...
            return
        if self.sw_virtual_cam.isChecked():
            self._send_to_vcam(bgr_frame)
        bgr_frame = fit_image_to_size(bgr_frame, self.width(), self.height())
        self._image_label.setPixmap(_bgr_to_qpixmap(bgr_frame))

    def _on_virtual_cam_toggled(self, checked: bool) -> None:
        if not checked:
            self._close_vcam()

    def _send_to_vcam(self, frame: np.ndarray) -> None:
        h, w = frame.shape[:2]
        if self._vcam is not None and (self._vcam.width, self._vcam.height) != (w, h):
            self._close_vcam()
        if self._vcam is None:
            try:
                self._vcam = pyvirtualcam.Camera(
                    width=w, height=h, fps=round(self._cam_fps) or 30,
                    fmt=pyvirtualcam.PixelFormat.BGR,
                )
                update_status(f"Virtual camera started: {self._vcam.device}")
            except Exception as e:
                update_status(f"Virtual camera unavailable: {e}")
                self.sw_virtual_cam.setChecked(False)
                self.sw_virtual_cam.setEnabled(False)
                return
        try:
            self._vcam.send(frame)
        except Exception as e:
            update_status(f"Virtual camera error: {e}")
            self._close_vcam()
            self.sw_virtual_cam.setChecked(False)
