...
```bash
git clone --depth 1 https://github.com/diyasharma200822-cloud/AC-DEEP-CAM.git
cd AC-DEEP-CAM
```
...
```bash
python3.14 run.py --execution-provider coreml --max-memory 0
```
...
- `--max-memory 0` is required on macOS. `resource.setrlimit(RLIMIT_DATA, ...)` in
  `modules/core.py` always raises `ValueError: current limit exceeds maximum limit` on
  Darwin (the OS doesn't support resizing that limit) &mdash; passing `0` disables the
  memory cap and skips the call entirely. Omitting the flag crashes the app on startup.
...
The live preview window has a **"Send to Virtual Camera"** toggle that publishes the
swapped feed as a system webcam (via [pyvirtualcam](https://github.com/letmaik/pyvirtualcam)),
so any app &mdash; Zoom, Meet, FaceTime, Photo Booth &mdash; can select it as a camera source.
This is a one-time setup, and OBS itself never needs to be running afterward:

1. Install OBS (only its camera driver is used, not the app itself):
   ```bash
   brew install --cask obs
   ```
2. Open OBS once and click **Start Virtual Camera** (bottom-right of the main window).
   This triggers macOS to prompt for approval of the camera System Extension.
3. Go to **System Settings &rarr; Privacy & Security**, approve the extension, and
   restart if prompted. You can quit OBS after this &mdash; it doesn't need to stay open.
4. In Deep-Live-Cam: pick your real webcam from the camera dropdown (**not** "OBS
   Virtual Camera" &mdash; that's the output device, selecting it as input is circular
   and will show nothing), click **Live**, then check **Send to Virtual Camera**.
5. Verify the output in a separate app (e.g. Photo Booth) by selecting "OBS Virtual
   Camera" as its camera.
